...
export class PoolCapitalization extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save PoolCapitalization entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type PoolCapitalization must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("PoolCapitalization", id.toString(), this);
    }
  }

  static loadInBlock(id: string): PoolCapitalization | null {
    return changetype<PoolCapitalization | null>(
      store.get_in_block("PoolCapitalization", id),
    );
  }

  static load(id: string): PoolCapitalization | null {
    return changetype<PoolCapitalization | null>(
      store.get("PoolCapitalization", id),
    );
  }
  ...
  get capitalization(): BigDecimal {
    let value = this.get("capitalization");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBigDecimal();
  ...
  set capitalization(value: BigDecimal) {
    this.set("capitalization", Value.fromBigDecimal(value));
  }
}
